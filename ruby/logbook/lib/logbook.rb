...
class Logbook
  ...
  def current_task
    ...
    row = closest_task_from_cursor

    return nil if row == nil

    while (row <= current_buffer.count)
      case current_buffer[row]
      when /^(?:\[\d\d:\d\d\])?\s*\[(#{STATUS_REGEX})\] (.*)$/
        task[:status] = $1.strip
        task[:title] = $2.chomp.strip
      when /^.*\[ID: (.*)\].*$/
        task[:id] = $1.strip
      else
        break
  ...
  def closest_task_from_cursor
    row, _ = current_window.cursor

    while (row > 0)
      if current_buffer[row] =~ /^(\[\d\d:\d\d\])?\s*\[#{STATUS_REGEX}\] .*$/
        return row
      elsif current_buffer[row] =~ /^\[\d\d:\d\d\].*/
        return nil
      end
